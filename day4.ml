...
let part1 input =
    ...
    let bingo state =
        ...
        let rec inner numbers state =
          let update_scores number state =
            let update_single_score state number (board, score) =
          ...
          | number::tail_numbers ->
              ...
              match state.code with
                  None -> inner tail_numbers state
                | Some x -> x
        in
        inner state.game.numbers state
    in
    let state = build_state game in bingo state

let part2 _raw_input = 2